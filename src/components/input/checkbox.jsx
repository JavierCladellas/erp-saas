const CheckboxField = ({ name, value, onChange, label, externalError, variant = "classic", readOnly}) => {
    ...
    switch (variant) {
        case "classic":
            variantStyle = (
                <>
                    <input type="checkbox" name={name} checked={value} onChange={onChange} readOnly={readOnly}
        ...
        case "switch":
            variantStyle = (
                <label className="inline-flex relative items-center cursor-pointer gap-3">
                    <input name={name} type="checkbox" readOnly={readOnly} checked={value} onChange={onChange} className="sr-only peer" />
